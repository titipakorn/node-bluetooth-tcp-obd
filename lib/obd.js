...
OBDReader.prototype.autoconnect = function(type = "bluetooth", parameter) {
  this.connType = type;
  if (type == "TCP") OBDReader.prototype.autoconnectTCP(parameter, this);
  else OBDReader.prototype.autoconnectBluetooth(parameter, this);
};
  ...
  bluetoothSerial.list(
    function(devices) {
      devices.forEach(function(device) {
        var nameMatch =
          !query || device.name.replace(/\W/g, "").search(search) != -1;
        if (nameMatch) {
          self.emit(
            "debug",
            "Found device: " +
          ...
          self.emit(
            "debug",
            "Ignoring device: " + device.name + " (" + device.address + ")"
          );
        }
      });
    },
...
OBDReader.prototype.disconnect = function(cb) {
  ...
  if (this.connType == "bluetooth")
    bluetoothSerial.disconnect(
      () => {
        this.emit("success", "disconnected");
      },
      err => {
